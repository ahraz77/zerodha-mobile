...
function StockModal({ isOpen, onClose, stock, onViewChart, mode }) {
  ...
  // Handle direct mode opening (for chart B/S buttons)
  useEffect(() => {
    if (isOpen && mode) {
      if (mode === 'buy') {
        setOrderType('BUY');
  ...
  return (
    <>
      ...
      {/* Modal */}
      <div className="fixed inset-x-0 bottom-0 z-50 bg-white animate-slide-up" style={{ height: '50vh' }}>
        {/* Stock Header */}
        <div className="px-4 py-3 bg-white">
          <div className="text-[20px] font-medium text-black">{stock.name}</div>
          <div className="flex items-center mt-1">
            <span className="text-[13px] text-gray-500 mr-2">{stock.exch}</span>
            <span className="text-[16px] font-medium text-black mr-2">{stock.price?.toFixed(2)}</span>
            <span className="text-[13px]" style={{ color: changeColor }}>
              {stock.change >= 0 ? '' : ''}{stock.change?.toFixed(2)} ({stock.pct >= 0 ? '' : ''}{stock.pct?.toFixed(2)}%)
            </span>
          </div>
        </div>
